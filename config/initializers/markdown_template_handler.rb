...
class EmailHtmlRenderer < Redcarpet::Render::HTML
  ...
  def link(link, title, content)
    if content.is_a?(String) && content.starts_with?('[') && content.ends_with?(']')
      content = content[1...-1]
      style = "background-color: #{button_color}; font-size: 16px; text-decoration: none; padding: 8px 10px; " \
        ...
        <tbody>
          <tr>
            <td align="center" style="padding: 10px 0;">
              <a href="#{link}" class="btn-link" style="#{style}" bgcolor="#{button_color}">
                  <!--[if mso]>
  ...
  def button_color
    Visa::Config.oevkg? ? '#cc0000' : '#6894c8'
  end
end
